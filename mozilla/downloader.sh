...
curl --max-time 30 --retry-delay 3 --retry 10 -4s -# https://raw.githubusercontent.com/v2fly/domain-list-community/refs/heads/master/data/mozilla | sed 's/ //g' | sed -r '/^\s*$/d' | sed '/!/d' | sed '/!!/d' | sed '/#/d' | sed 's/DOMAIN-SUFFIX,//g' | sed 's/^https\?:\/\///g' | sed 's/full://g' | sed '/IP-CIDR/d' | sed '/@/d' | sed '/:/d' >> /tmp/"$1"_domain.txt
...
#get_prefix 'mozilla' || echo 'failed'

# save ipv4
#grep -v ':' /tmp/mozilla.txt | sed 's/\/32//g' > /tmp/mozilla-ipv4.txt

# save ipv6
#grep ':' /tmp/mozilla.txt > /tmp/mozilla-ipv6.txt

# Create/Prepare ACL List for Shadowsocks IPv4
echo -e "[bypass_all]\n[proxy_list]" | tee mozilla/ipv4.acl

# Create/Prepare ACL List for Shadowsocks IPv6
#echo -e "[bypass_all]\n[proxy_list]" | tee mozilla/ipv6.acl

add_domain 'mozilla' || echo 'failed'

# sort & uniq
#sort -h /tmp/mozilla-ipv4.txt | uniq >> mozilla/ipv4.acl
#sort -h /tmp/mozilla-ipv6.txt | uniq >> mozilla/ipv6.acl
