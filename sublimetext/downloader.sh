...
add_domain() {
curl --max-time 30 --retry-delay 3 --retry 10 -4s -# https://raw.githubusercontent.com/blackmatrix7/ios_rule_script/master/rule/Shadowrocket/SublimeText/SublimeText.list | sed 's/ //g' | sed -r '/^\s*$/d' | sed '/!/d' | sed '/!!/d' | sed '/#/d' | sed 's/DOMAIN-SUFFIX,//g' | sed 's/^https\?:\/\///g' | sed '/IP-CIDR/d' | sed '/@/d' | sed 's/full://g' | sed '/:/d' > /tmp/"$1"_domain.txt
...
#get_prefix 'sublimetext' || echo 'failed'

# save ipv4
#grep -v ':' /tmp/sublimetext.txt | sed 's/\/32//g' > /tmp/sublimetext-ipv4.txt

# save ipv6
#grep ':' /tmp/sublimetext.txt > /tmp/sublimetext-ipv6.txt

# Create/Prepare ACL List for Shadowsocks IPv4
echo -e "[bypass_all]\n[proxy_list]" | tee sublimetext/ipv4.acl

# Create/Prepare ACL List for Shadowsocks IPv6
#echo -e "[bypass_all]\n[proxy_list]" | tee sublimetext/ipv6.acl

add_domain 'sublimetext' || echo 'failed'

# sort & uniq
#sort -h /tmp/sublimetext-ipv4.txt | uniq >> sublimetext/ipv4.acl
#sort -h /tmp/sublimetext-ipv6.txt | uniq >> sublimetext/ipv6.acl
