...
add_domain() {
curl --max-time 30 --retry-delay 3 --retry 10 -4s -# https://raw.githubusercontent.com/v2fly/domain-list-community/master/data/ruby | sed 's/ //g' | sed -r '/^\s*$/d' | sed '/!/d' | sed '/!!/d' | sed '/#/d' | sed 's/DOMAIN-SUFFIX,//g' | sed 's/^https\?:\/\///g' | sed 's/full://g' | sed '/IP-CIDR/d' | sed '/@/d' | sed '/:/d' > /tmp/"$1"_domain.txt
...
#get_prefix 'ruby' || echo 'failed'

# save ipv4
#grep -v ':' /tmp/ruby.txt | sed 's/\/32//g' > /tmp/ruby-ipv4.txt

# save ipv6
#grep ':' /tmp/ruby.txt > /tmp/ruby-ipv6.txt

# Create/Prepare ACL List for Shadowsocks IPv4
echo -e "[bypass_all]\n[proxy_list]" | tee ruby/ipv4.acl

# Create/Prepare ACL List for Shadowsocks IPv6
#echo -e "[bypass_all]\n[proxy_list]" | tee ruby/ipv6.acl

add_domain 'ruby' || echo 'failed'

# sort & uniq
#sort -h /tmp/ruby-ipv4.txt | uniq >> ruby/ipv4.acl
#sort -h /tmp/ruby-ipv6.txt | uniq >> ruby/ipv6.acl
